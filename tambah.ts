...
function kurang(c: number, d: number): number {
    return c - d
}
console.log(kurang(3, 1));

function persegiPanjang(p: number, l: number): number {
    let keliling =  2 * (p + l)
    return keliling
}
...
function konveriTanda(tanda: string): string {
    // * = ini lambang bintang
    // # ini pagar
    // @ 
    // &
...
function remoteTv(status: boolean) {
    tv = !status // true
    if (tv != false) { //
...
function remotKipas(status: boolean) {
    kipas = status
    if (kipas == true) {
...
function remoteTvNested(parameterStatusTv: boolean, parameterVolumeTv: number) {
    let statusTv: boolean = parameterStatusTv
    let volumeTv: number = parameterVolumeTv
    if (statusTv == false) {
...
function mesinMotorNested(parameterStatusMesin: boolean, parameterStatusPanas: boolean, porseneling: number) {
    let statusMotor: boolean = parameterStatusMesin
    let statusPanasMotor: boolean = parameterStatusPanas
    let statusPorseneling: number = porseneling
    let statusBerjalan: boolean = true
